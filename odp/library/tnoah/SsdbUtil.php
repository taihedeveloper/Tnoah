 ...
 */
class Tnoah_SsdbUtil extends SSDB
{
	function __construct($param, $timeout_ms=100){
		$host = $param["host"];
		$port = $param["port"];
...
class SSDB
{
	...
	function __construct($host, $port, $timeout_ms=2000){
		...
		if(!$this->sock){
			//Dapper_Log::fatal("S_ssdb_obj_create_fail_{$host}", 'dal');
		}
	...
	function __call($cmd, $params=array()){
		...
		if($this->async_auth_password !== null){
			...
			if($auth !== true){
				//Dapper_Log::fatal("S_ssdb_obj_Authentication failed", 'dal');
			    return false;
			}
		}
		...
		try{
			if($this->send_req($cmd, $params) === false){
				...
				//::fatal("S_ssdb_obj_send error".$cmd.$params, 'dal');
			}else{
				$resp = $this->recv_resp($cmd, $params);
			}
		}catch(SSDBException $e){
			//Dapper_Log::fatal("S_ssdb_obj_".$cmd.$params, 'dal');
		}

		if($resp->code == 'noauth'){
			$msg = $resp->message;
			//Dapper_Log::fatal("S_ssdb_obj_".$msg, 'dal');
		}
        //记录日志
		if($resp->code != "ok"){
			//Dapper_Log::fatal("S_ssdb_obj".$cmd."_fail".serialize($params), 'dal');
		}else{
			//Dapper_Log::notice("S_ssdb_obj_".$cmd."success".serialize($params), 'dal');
		}
		//end
		$resp = $this->check_easy_resp($cmd, $resp);
		...
		switch($cmd){
			...
			case 'expire':
				if($resp[0] == 'ok'){
					...
					var_dump($resp);
					$errmsg = isset($resp[1])? $resp[1] : '';
					//Dapper_Log::fatal("S_ssdb_obj_".$errmsg, 'dal');
					return new SSDB_Response($resp[0], $errmsg);
				}
			case 'zavg':
				if($resp[0] == 'ok'){
				...
				if($resp[0] == 'ok'){
					...
					if($size <= 1){
						if(count($resp) == 2){
							...
							//Dapper_Log::fatal("S_ssdb_obj_"."Invalid response", 'dal');
							return new SSDB_Response('server_error', 'Invalid response');
						}
					}else{
	...
	function send($data){
		...
		try{
			while(true){
				...
				if($ret === false || $ret === 0){
					$this->close();
					//Dapper_Log::fatal("S_ssdb_obj_Connection lost", 'dal');
				}
				$s = substr($s, $ret);
				if(strlen($s) == 0){
					break;
			...
			$this->close();
			//Dapper_Log::fatal("S_ssdb_obj_".$e, 'dal');
		}
		return $ret;
	}
			...
			if($ret === null){
				...
				if($data === false || $data === ''){
					if(feof($this->sock)){
						...
						//Dapper_Log::fatal("S_ssdb_obj_Connection timeout", 'dal');
					}
				}
				$this->recv_buf .= $data;
