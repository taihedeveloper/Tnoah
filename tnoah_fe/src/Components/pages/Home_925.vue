<template>
	<div class="home">
		<div class="topbox">
			<div class="selectbox" style="float:left">
				<div class="title">机器
				</div>
				<div class="content">
					<div class="contentHead clearfix">
						<input type="text" id="searchMechine"><span id="searchM"></span>
						<ul>
							<li @click="checkten()">+10</li>
							<li @click="checkall()">全选</li>
							<li @click="uncheckips()">取消</li>
						</ul>
					</div>
					<div class="contentTable">
						<table >
							<tbody>
								<tr v-for="(value,index) in mechineTable">
									<td style="width:2%"><input type="checkbox" name="check-ip" v-on:click="getParams()"></td>
									<td  style="width:10%">{{value.ip_addr}}</td>
									<td  style="width:20%">{{value.name}}</td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>
			</div>
			<div class="selectbox" style="float:right">
				<div class="title">监控项
					<input type="text" id="searchItem"><span id="searchI"></span>
				</div>
				<div class="content">
					<div class="contentHead clearfix">
						<ul>
							<li id="CPU" @click="getCpu()">CPU</li>
							<li id="MEM" @click="getMem()">内存</li>
							<li id="DISK" @click="getDisk()">磁盘空间</li>
							<li id="DISK_IO" @click="getDiskIo()">磁盘IO</li>
							<li id="NET" @click="getNet()">网卡IO</li>
							<li id="PROCESS" @click="getProcess()">进程</li>
							<li id="BUSINESS" @click="getBusiness()">业务</li>
							<li id="PORT" @click="getPort()">端口</li>
						</ul>
					</div>
					<div class="contentTable" id="contentTable">
						<table >
							<tbody>
								<tr v-for="(value,index) in monitorItemTable">
									<!-- <td style="width:2%"><input type="checkbox"  name="check-item" @change="getParams()" v-on:click = "checked($event)"></td> -->
									<td style="width:2%"><input type="checkbox"  name="check-item" v-on:click="getParams()"></td>
									<!-- <td style="width:2%"><input type="checkbox"  name="check-item"></td> -->
									<td  style="width:25%">{{value}}</td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>
			</div>
		</div>
		<div class="graphbox" style="clear:both;">
			<div class="graphHead">
				<ul>
					<li>趋势图</li>
					<li>
						<el-date-picker v-model="value1" type="datetime" placeholder="开始时间" @change="pickStartTime">
    					</el-date-picker>
					</li>
					-
					<li>
						<el-date-picker v-model="value2" type="datetime" placeholder="结束时间" @change="pickEndTime">
    					</el-date-picker>
					</li>
					<li id="oneHour" @click="setOneHour" class="cur">1小时</li>
					<li id="oneDay" @click="setOneDay">1天</li>
					<li id="threeDay" @click="setThreeDay">3天</li>
					<li id="sevenDay" @click="setSevenDay">7天</li>
					<li><button class="button" @click="getParams()">确定</button></li>
					<li><button class="button" @click="setOneHour()">取消</button></li>
				</ul>
			</div>
		</div>
<!-- 		<Graph :params="params">
		</Graph> -->
		<div id="box-check">
			<!-- <div class="graph">
				<span class="close"></span>
				<div id="graph0">aaaaaaaaaaaaaaaaaaaaaa</div>
			</div> -->
		</div>
	</div>

</template>
<script>
import {mapGetters,mapActions} from 'vuex'
// import {DatePicker} from 'element-ui'
import vueGetData from "../../Js/vueGetData.js"
// import Graph from './Graph.vue'
import echarts from "../../Js/plugins/echarts.js"

	export default{
		name:'home',
		data () {
			return {
				mechineTable: {},
				monitorItems:{},//存储某个机器下的所有监控项
				currentItem:'CPU',//存储某个机器下的当前监控tag
				monitorItemTable: [],//存储某个机器下的当前监控tag下的监控项
				value1:'',//开始时间的日期形式
				value2:'',//结束时间的日期形式
				startTime:'',//开始时间的时间戳形式
				endTime:'',//结束时间的时间戳形式
				currentTime:'oneHour',//存储当前时间快捷方式
				selectIps:[],
				selectItems:[],
				params:{
				}
			}
		},
		computed:mapGetters({
			treeId:"treeId",
		}),
		watch:{
			treeId: function(){
				let data = {"group_id":this.treeId}
				vueGetData.getData("getmachineinfos",data,function(jsondata){
		        	if(jsondata.body.error_code === 22000){
		        		this.mechineTable = jsondata.body.data;
		        	}else{
		        		this.mechineTable = {}
		        	}
		        }.bind(this),function(){

		        }.bind(this));

		        vueGetData.getData("getgroupmetricsname",data,function(jsondata){
		        	if(jsondata.body.error_code === 22000){
		        		let json = jsondata.body.data;
	        			for(var item in json){
	        				this.monitorItems[item]=json[item];
	        			}
	        			this.currentItem='CPU';
	        			this.monitorItemTable=json[this.currentItem]
	        			document.getElementById(this.currentItem).setAttribute("class","cur")
		        	}
		        }.bind(this),function(){

		        }.bind(this));
			},
			// selectItems:function(){
			// 	console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
			// 	this.getParams()
			// }
		},
		methods: {
			// checked:function(event){
			// 	var body=document.getElementById("box-check");
			// 	var html="";
			// 	var contentTable=document.getElementById("contentTable");
			// 	var obj=document.getElementsByName("check-item");
								
			//     var items=document.getElementsByName("check-item");
			// 	var itemLen=items.length;
			// 	if(itemLen>0){
			// 		var selectItems=[];
   //              	for(var i=0;i<itemLen;i++){
   //              		if(items[i].checked){
   //              			var row = items[i].parentNode.parentNode;
   //              			var content=row.cells[1].innerHTML;
   //              			selectItems.push(content)
   //              		}
   //              	}
   //              	console.log(selectItems)
   //              	for(var i=0;i<selectItems.length;i++){
   //              		html+="<div id=graph"+i+" class="+"graph"+">新元素</div>"
   //              		body.innerHTML = html;
   //              	}
   //              }
			// },
			checkten:function(){
				var ips=document.getElementsByName("check-ip");
                var len=ips.length;
                if(len>10){
                	for(var i=0;i<10;i++){
                		ips[i].checked=true;
                	}
                }else{
                	for(var i=0;i<len;i++){
                		ips[i].checked=true;
                	}
                }
                // let data = {"startTime":'1502867005',"endTime":'1502877805',"selectIps":['***.***.***.***','***.***.***.***'],"selectItems":'cost',"type":'sec'}
                // this.params = data;

    //             var timestamp = '1425553097';
				// var d = new Date(timestamp * 1000);    //根据时间戳生成的时间对象
				// var date = (d.getHours()) + ":" + 
				//            (d.getMinutes()) + ":" + 
				//            (d.getSeconds());
				// console.log(this.startTime)
			},
			checkall:function(){
                var ips=document.getElementsByName("check-ip");
                var len=ips.length;
                if(len>0){
                	for(var i=0;i<len;i++){
                		ips[i].checked=true;
                	}
                }
			},
			uncheckips:function(){
                var ips=document.getElementsByName("check-ip");
                var len=ips.length;
                if(len>0){
                	for(var i=0;i<len;i++){
                		ips[i].checked=false;
                	}
                }
			},
			uncheckitems:function(){
				var items=document.getElementsByName("check-item");
                var len=items.length;
                if(len>0){
                	for(var i=0;i<len;i++){
                		items[i].checked=false;
                	}
                }
			},
			getCpu:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='CPU';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getMem:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='MEM';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getDisk:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='DISK';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getDiskIo:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='DISK_IO';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getNet:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='NET';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getProcess:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='PROCESS';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getBusiness:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='BUSINESS';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			getPort:function(){
				document.getElementById(this.currentItem).setAttribute("class","")
				this.currentItem='PORT';
    			this.monitorItemTable=this.monitorItems[this.currentItem]
    			document.getElementById(this.currentItem).setAttribute("class","cur")
    			this.uncheckitems();
			},
			pickStartTime:function(){
				this.startTime=Date.parse(this.value1)/1000;
				console.log(this.startTime)
			},
			pickEndTime:function(){
				this.endTime=Date.parse(this.value2)/1000;
				console.log(this.endTime)
			},
			setOneHour:function(){
				document.getElementById(this.currentTime).setAttribute("class","")
				this.currentTime='oneHour';
    			document.getElementById(this.currentTime).setAttribute("class","cur")
    			//获取当前时间
				var endtimestamp = Date.parse(new Date())/1000;
				this.value2 = this.getFormatDate(endtimestamp)
				this.endTime = '';
				console.log("end time:value2")
				console.log(this.value2)
				console.log(this.endTime)
				var starttimestamp = Date.parse(new Date())/1000-3600;
				this.value1 = this.getFormatDate(starttimestamp)
				this.startTime = '';
				console.log("start time:value1")
				console.log(this.value1)
				console.log(this.startTime)
			},
			setOneDay:function(){
				document.getElementById(this.currentTime).setAttribute("class","")
				this.currentTime='oneDay';
    			document.getElementById(this.currentTime).setAttribute("class","cur")
    			//获取当前时间
				var endtimestamp = Date.parse(new Date())/1000;
				this.value2 = this.getFormatDate(endtimestamp)
				this.endTime = '';
				console.log("end time:value2")
				console.log(this.value2)
				console.log(this.endTime)
				var starttimestamp = Date.parse(new Date())/1000-86400;
				this.value1 = this.getFormatDate(starttimestamp)
				this.startTime = '';
				console.log("start time:value1")
				console.log(this.value1)
				console.log(this.startTime)
			},
			setThreeDay:function(){
				document.getElementById(this.currentTime).setAttribute("class","")
				this.currentTime='threeDay';
    			document.getElementById(this.currentTime).setAttribute("class","cur")
    			//获取当前时间
				var endtimestamp = Date.parse(new Date())/1000;
				this.value2 = this.getFormatDate(endtimestamp)
				this.endTime = '';
				console.log("end time:value2")
				console.log(this.value2)
				console.log(this.endTime)
				var starttimestamp = Date.parse(new Date())/1000-259200;
				this.value1 = this.getFormatDate(starttimestamp)
				this.startTime = '';
				console.log("start time:value1")
				console.log(this.value1)
				console.log(this.startTime)
			},
			setSevenDay:function(){
				document.getElementById(this.currentTime).setAttribute("class","")
				this.currentTime='sevenDay';
    			document.getElementById(this.currentTime).setAttribute("class","cur")
    			//获取当前时间
				var endtimestamp = Date.parse(new Date())/1000;
				this.value2 = this.getFormatDate(endtimestamp)
				this.endTime = '';
				console.log("end time:value2")
				console.log(this.value2)
				console.log(this.endTime)
				var starttimestamp = Date.parse(new Date())/1000-604800;
				this.value1 = this.getFormatDate(starttimestamp)
				this.startTime = '';
				console.log("start time:value1")
				console.log(this.value1)
				console.log(this.startTime)
			},
			getParams:function(){
				//获取监控项个数
				this.selectItems=[];
				this.selectIps=[];
				var items=document.getElementsByName("check-item");
				var itemLen=items.length;

				if(itemLen>0){
                	for(var i=0;i<itemLen;i++){
                		if(items[i].checked){
                			var row = items[i].parentNode.parentNode;
                			var content=row.cells[1].innerHTML;
                			this.selectItems.push(content)
                		}
                	}
	                if(this.selectItems.length>0){
	                	console.log("select")
	                	var ips=document.getElementsByName("check-ip");
						var ipLen=ips.length;
		                if(ipLen>0){
		                	//检查是否选择ip
		                	for(var i=0;i<ipLen;i++){
		                		if(ips[i].checked){
		                			var row = ips[i].parentNode.parentNode;
		                			var content=row.cells[1].innerHTML;
		                			this.selectIps.push(content);
		                		}
		                	}
		                	if(this.selectIps.length>0){
			                	var body=document.getElementById("box-check");
								var html="";
							    var items=document.getElementsByName("check-item");
								var itemLen=items.length;
		                		for(var i=0;i<this.selectItems.length;i++){
			                		// html+="<div id=graph"+i+" class="+"graph"+"></div>"
			                		// html+="<div class="+"graph"+"><span class="+"closeGraph"+"></span><div id=graph"+i+"></div></div>"
			                		html+='<div class="graph"><span class="close" id="close'+i+'"></span><div id="graph'+i+'" class="echartsGraph"></div></div>';
			                		body.innerHTML = html;
			                		document.getElementById('close'+i).onclick=this.closeGraph(i);
			                		// document.getElementById('close'+i).addEventListener("click",this.closeGraph(i));
			                		console.log("-------------------------")
			                		console.log(document.getElementById("graph0"));
			                		this.drawGraph(i)
			                	}
			                }
		                }
	                }else{
	                	var body=document.getElementById("box-check");
						var html="";
						body.innerHTML = html;
	                }
                }
			},
			drawGraph:function(index){
				let ips=this.selectIps[0];
				for(var i=1;i<this.selectIps.length;i++){
					ips += ","+this.selectIps[i];
				}

				let data={
					"starttime" : this.startTime,
					"endtime" : this.endTime,
					"ips" : ips,
					"cols" : this.selectItems[index],
					"type" : 'sec'
				}
				vueGetData.getData("gethbasedata",data,function(jsondata){
					if(jsondata.body.length==0){
						console.log("数据为空")

						var id = 'graph'+index;
						var myChart = echarts.init(document.getElementById(id));
						var option = {
					        title: {
						        text: this.selectItems[index]
						    },
						    tooltip: {
						        trigger: 'axis'
						    },
						    legend: {
						        orient: 'vertical',
								x: 'right',
						        data:this.selectIps
						    },
						    xAxis: {
						    	type: 'category',
	        					boundaryGap: false,
	        					data: []
						    },
						    yAxis: {
						        splitLine: {
						            show: false
						        }
						    },
						    toolbox: {
						        left: 'center',
						        feature: {
						            dataZoom: {
						                yAxisIndex: 'none'
						            },
						            restore: {},
						            saveAsImage: {}
						        }
						    },
						    dataZoom: [{
						        startValue: startIndex
						    }, {
						        type: 'inside'
						    }],

						    series: []
					    };
					    myChart.setOption(option);
					}else{
						//获取折线图横坐标数据
						let xData=[];
						for (var start=this.startTime;start<=this.endTime;start++){
							var d = new Date(start * 1000);    //根据时间戳生成的时间对象
							var time = (d.getHours()) + ":" + 
							           (d.getMinutes()) + ":" + 
							           (d.getSeconds());
						    xData.push(time);
						}
						var d = new Date(start * 1000);    //根据时间戳生成的时间对象
						var startIndex = (d.getHours()) + ":" + 
						           (d.getMinutes()) + ":" + 
						           (d.getSeconds());
						//获取折线图纵坐标数据
						var ips=[];
						let yData = [];
						for (var i=0;i<this.selectIps.length;i++){
							//对应ip
							var ip=this.selectIps[i];
							//ip对应接口数据
							let resultData = jsondata.body[this.selectItems[index]];
							//接口数据转换为数组形式
							let tempY = []
							for(var item in resultData[ip]){
								var d = new Date(item * 1000);
								var time = (d.getHours()) + ":" + 
							           (d.getMinutes()) + ":" + 
							           (d.getSeconds());
								tempY.push([time,parseFloat(resultData[ip][item])])
							}
							//图标读取数据
							let tempData={
								name: ip,
						        type: 'line',
						        smooth: true,
						        data:tempY
							}
							yData.push(tempData)
						}
						var id = 'graph'+index;
						var myChart = echarts.init(document.getElementById(id));
						var option = {
					        title: {
						        text: this.selectItems[index]
						    },
						    tooltip: {
						        trigger: 'axis'
						    },
						    legend: {
						        orient: 'vertical',
								x: 'right',
						        data:this.selectIps
						    },
						    xAxis: {
						    	type: 'category',
	        					boundaryGap: false,
	        					data: xData
						    },
						    yAxis: {
						        splitLine: {
						            show: false
						        }
						    },
						    toolbox: {
						        left: 'center',
						        feature: {
						            dataZoom: {
						                yAxisIndex: 'none'
						            },
						            restore: {},
						            saveAsImage: {}
						        }
						    },
						    dataZoom: [{
						        startValue: ''
						    }, {
						        type: 'inside'
						    }],

						    series: yData
					    };
					    myChart.setOption(option);
					}
		        }.bind(this),function(){

		        }.bind(this));
			},
			getFormatDate:function(timestamp){
				var date = new Date(timestamp * 1000);    //根据时间戳生成的时间对象
			    var currentdate = date.getFullYear() + "-" + date.getMonth() + "-" + date.getDate()
			            + " " + date.getHours() + ":" + date.getMinutes()
			            + ":" + date.getSeconds();
			    // console.log(currentdate)
			    return currentdate;
			},
			closeGraph:function(index){
				console.log("++++++++++++++++++++++++++++++++++++++")
			}
		},
		created(){
			// vueGetData.getData("getmetrics",{},function(jsondata){
	  //       	if(jsondata.body.error_code === 22000){
	  //       		this.monitorItemTable = jsondata.body.data;
	  //       	}else{
	  //       		this.monitorItemTable = {};
	  //       	}
	  //       }.bind(this),function(){

	  //       }.bind(this));

			//获取当前时间
			var endtimestamp = Date.parse(new Date())/1000;
			this.value2 = this.getFormatDate(endtimestamp)
			this.endTime = '';
			console.log("end time:value2")
			console.log(this.value2)
			console.log(this.endTime)
			var starttimestamp = Date.parse(new Date())/1000-3600;
			this.value1 = this.getFormatDate(starttimestamp)
			this.startTime = '';
			console.log("start time:value1")
			console.log(this.value1)
			console.log(this.startTime)
		},
		components: {
			// 'el-date-picker': DatePicker,
		}


	}
</script>
<style lang="less" >
@import "../../Css/mixin.less";
.el-input__icon+.el-input__inner {
    padding-right: 35px;
    width: 180px;
}
.el-input--small .el-input__inner {
    height: 30px;
    margin: 0px;
    width: 140px;
}
.home {
	padding-left: 240px;
	width: 100%;
	background-color:#ffffff;
}
.topbox {
	.selectbox {
		width: 50%;
		height: 300px;
		padding: 10px 20px 0px 20px;

		.title {
		    display: block;
		    position: relative;
		    line-height: 30px;
		    font-size: 18px;
		    font-weight: bold;
		    color: #000;

		    #searchItem {
				float: right;
				height: 28px;
				width: 200px;
				font-size: 15px;
				font-weight: normal;
				border: 1px solid #3c5876;
			}
			#searchI {
				position: absolute;
				top: 5px;
				right: 20px;
				width: 20px;
				height: 20px;
				background:url(@searchico) no-repeat;
				background-size:20px;
				cursor:pointer;
			}
		}
	}
}

.content {
	width: 100%;
	height: 260px;
	background-color:#eee;
	border: 1px solid #000;
}

.contentHead {
	height: 40px;
	background-color: #354f65;
	position: relative;
	@h: 30px;
	line-height: @h;
	border-bottom: 1px solid #3c5876;
	padding: 5px 0;
	font-size: 15px;

	#searchMechine {
		float:left;
		height: @h;
		width: 200px;
		border: 1px solid #3c5876;
	}
	#searchM {
		position: absolute;
		top: 10px;
		left: 180px;
		width: 20px;
		height: 20px;
		background:url(@searchico) no-repeat;
		background-size:20px;
		cursor:pointer;
	}

	li {
		display: inline;
		color: #fff;
		//text-decoration: underline;
		margin:0 10px;
		cursor:pointer;
	}
}

.contentTable{
	height:210px;
	overflow-y:auto;

	td{
		height:30px;
		line-height:30px;
		font-size:15px;
	}
}

.graphbox {
	margin: 20px;
	padding-top:20px;
	font-size: 15px;
	color: #fff;

	.graphHead{
		height: 40px;
		background-color: #354f65;
		line-height: 30px;
		padding: 5px 0;
	}
	li {
		display: inline;
		margin:0 10px;
	}
}

.graph {
	//margin: 20px;
	padding:20px;
	width:100%;
	position: relative;

	.echartsGraph{
		height:300px;
		width:90%;
	}

	.close{
		position: absolute;
		top: 0;
		right: 0;
		cursor: pointer;
		border-radius: 5px;
		width: 50px;
		height: 50px;

		&:after,&:before{
		    content: "";
		    position: absolute;
		    width: 2px;
		    height: 13px;
		    background: #bbbbbb;
		    font-size:0;
		    line-height:0;
		    vertical-align:middle;
		    transform: rotate(45deg);
		    -webkit-transform: rotate(45deg);
		    transform-origin: 50% 50%;
		    -webkit-transform-origin: 50% 50%;
		    left: 24px;
		    top: 18px;
		}
		&:after{
    		-webkit-transform: rotate(-45deg);
		}
	}
}

.button {
    height: 30px;
    padding: 0 10px;
    line-height: 28px;
    background-color: #4085f4;
    border-radius: 3px;
    color: #fff;
    font-size: 13px;
    display: inline-block;
    border: 1px solid #4085f4;
    position: relative;
    cursor:pointer;
}
.button:hover, .button:focus {
    color: #fff
}

.cur{
	font-weight: bold;
	text-decoration: underline;
}
</style>