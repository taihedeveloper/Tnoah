							...
							<table >
								<tbody>
									<tr v-for="(value,index) in memItemTable">
										...
										<td  style="width:12%" :title="value.name">{{value.name}}</td>
										<td  style="width:10%" :title="value.addi">{{value.addi}}</td>
									</tr>
								</tbody>
							</table>
						</div>
		...
		methods: {
			...
			drawGraph:function(index){
				...
				vueGetData.getData("gethbasedata",data,function(jsondata){
					if(jsondata.body.error === 22001){
					...
					}else if(jsondata.body.length==0){
						console.log("数据为空")
						//***************************加接口错误信息提示******************************
						var id = 'graph'+index;
						var myChart = echarts.init(document.getElementById(id));
						...
						xTamp.sort();
						//横坐标数据去重
						this.uniqueArray(xTamp);

						let xData=[];
						for(var i=0;i<xTamp.length;i++){
							var d = new Date(xTamp[i] * 1000);
			...
			drawGraph:function(index){
				...
				data["cols"]=this.selectItems[index];
				data["pid"]=this.treeId;
					...
					vueGetData.getData("gethbasedata",data,function(jsondata){
						if(jsondata.body.error === 22001){
							...
							if(i==this.selectIps.length){
								...
								let xData=[];
								for(var j=0;j<xTamp.length;j++){
									var d = new Date(xTamp[j] * 1000);
									var time = (d.getMonth()+1) + "-" + 
							           (d.getDate()) + " " + (d.getHours()) + ":" + 
							           (d.getMinutes()) + ":" + 
							           (d.getSeconds());
