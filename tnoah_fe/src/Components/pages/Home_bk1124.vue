				...
				<div class="graphHead">
					<ul>
						...
						<li>
	    					...
	    					<!-- <el-date-picker v-model="value1" align="right" type="datetime" placeholder="开始时间" :editable="canEdit" :picker-options="pickerOptions1"> </el-date-picker> -->
						</li>
						-
						<li>
							<el-date-picker v-model="value2" type="datetime" placeholder="结束时间" @change="pickEndTime">
	    					</el-date-picker>
	    					<!-- <el-date-picker v-model="value2" type="datetime" placeholder="结束时间" :editable="canEdit" :picker-options="pickerOptions0">
 							</el-date-picker> -->
						</li>
					...
					<el-date-picker v-model="startDate" align="right" type="datetime" placeholder="选择日期" :editable="canEdit" :picker-options="pickerOptions1"> </el-date-picker>
 					<label for="">至</label>
 					<el-date-picker v-model="endDate" type="datetime" placeholder="选择日期" :editable="canEdit" :picker-options="pickerOptions0">
 					</el-date-picker> -->
		...
		data () {
			return {
				...
				pickerOptions0: {
			        disabledDate(time) {
						// 最多只能选择一个月的
						let amonthAgo = Date.now() - 2592000000
						return time.getTime() > Date.now() || time.getTime() < amonthAgo  || (vueIns.startDate && vueIns.startDate.getTime && (time.getTime() < vueIns.startDate.getTime()))
			        }
			    },
			    pickerOptions1: {
			        disabledDate(time) {
			            // 最多只能选择一个月的
			            let amonthAgo = Date.now() - 2592000000
			            return time.getTime() > Date.now() || time.getTime() < amonthAgo || (vueIns.endDate && vueIns.endDate.getTime && (time.getTime() > vueIns.endDate.getTime()))
			        }
			    },
			 //    startDate: '',// 开始时间  
				// endDate: '', //结束时间 
